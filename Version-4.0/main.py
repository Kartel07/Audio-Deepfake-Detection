...
from Train import train_rawnet_model
from Test import evaluate_rawnet_eer

def parse_args():
    parser = argparse.ArgumentParser(description="Version 4.0 RawNet2 Execution Matrix")
...
def main():
    args = parse_args()
    print(f"\n--- Starting Version 4.0 RawNet2 System Matrix ---")
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    
    weight_filename = f"rawnet2_deepfake_detector_v4_{args.train_on}.pt"
    ...
    if args.mode == "train":
        ...
        train_rawnet_model(train_loader, epochs=args.epochs, save_path=weights_path)
        ...
        export_path = os.path.join(root_dir, "Paper1_Data_Exports", f"raw_scores_V4_Train{args.train_on}_Test{args.test_on}.csv")
        os.makedirs(os.path.dirname(export_path), exist_ok=True)
        evaluate_rawnet_eer(model, test_loader, export_csv_path=export_path)
